...
static void set_time(struct ntptime *new)
{
	...
	printf("set time tv_set.tv_sec:%d\n",tv_set.tv_sec);
...
#define sizeof_incoming (sizeof incoming_word)
	...
	for (;;) {
		...
		++probes_sent;
		printf("recv from 11111111111111111111111111\n");
		pack_len=recvfrom(usd,incoming,sizeof_incoming,0,
		                  &sa_xmit,&sa_xmit_len);
		printf("recv from 22222222222222222222222222,pack_len:%d\n",pack_len);
		error = ntpc->goodness;
		if (pack_len<0) {
			perror("recvfrom");
...
void sync_time()
{
	...
	setup_transmit(usd, "61.129.42.44", NTP_PORT, &ntpc);
