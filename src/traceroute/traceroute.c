...
int global_ttl_num=0;
int init_flag = 0;
route_info_t* global_route_info_list;
int current_ttl;
int current_probe_num;
char** global_param;




int run (int argc, char *argv[],int* ttl_num, route_info_t** route_info_list) {
	...
	if(init_flag == 0){
		global_route_info_list = (route_info_t*)malloc(DEF_HOPS*sizeof(route_info_t));
		register_module();
		init_flag = 1;
	}
	global_ttl_num = 0;
	memset(global_route_info_list,0,DEF_HOPS*sizeof(route_info_t));

	CLIF_parse (argc, argv, option_list, arg_list,CLIF_MAY_JOIN_ARG | CLIF_HELP_EMPTY);
	...
	if (ops->options && opts_idx > 1) {
	    ...
	    CLIF_parse (opts_idx, opts, ops->options, 0, CLIF_KEYWORD);
	}
	...
	do_it ();
	*ttl_num = current_ttl+1;
	*route_info_list = global_route_info_list;
...
void trace_route(char* ip,int* ttl_num, route_info_t** route_info_list)
{
	if(init_flag == 0){
		global_route_info_list = (route_info_t*)malloc(DEF_HOPS*sizeof(route_info_t));
		global_param = calloc(1,2*sizeof(route_info_t*));
		global_param[0]=calloc(1,200);
		global_param[1]=calloc(1,200);
		init_flag = 1;
	}

	strcpy(global_param[0],"sender");
	strcpy(global_param[1],ip);
	run(2,global_param,ttl_num,route_info_list);
}
...
int main (int argc, char *argv[]) {
	...
	printf("do it 1111111111111111\n");
	do_it ();
	printf("do it 2222222222222222\n");
...
static void print_addr (sockaddr_any *res) {
	...
	if (noresolve)
		...
		strcpy(global_route_info_list[current_ttl].domain[current_probe_num],buf);
		strcpy(global_route_info_list[current_ttl].ip[current_probe_num],str);
	}
...
static void print_probe (probe *pb) {
	...
	{
		global_route_info_list[current_ttl].time[current_probe_num] = -1;
		printf (" *");
	}
	else {
	...
	if (pb->recv_time) {
		...
		global_route_info_list[current_ttl].time[current_probe_num] = (int)(diff * 1000);
	}
